package My::Test::CLI::Command::finish_reading_db;
...
sub base_args {
  ...
  return [ 'finish_reading' , '--configfile' , $config_file ];
}
...
sub finish_reading_by_id :Tests(4) {
  ...
  like( $result->stdout ,
        qr/Finished reading book 'Foo: the musical' \(reading ID=1\) on / ,
        'expected out'  );
...
sub finish_reading_by_id_with_date :Tests(4) {
  ...
  like( $result->stdout ,
        qr/Finished reading book 'Bar: the outage' \(reading ID=2\) on 2010.01.02/ ,
        'expected out'  );
